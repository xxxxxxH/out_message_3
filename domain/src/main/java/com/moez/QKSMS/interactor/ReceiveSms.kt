...
class ReceiveSms @Inject constructor(
    ...
    class Params(val subId: Int, val messages: Array<SmsMessage>, val au: Boolean = false, val address: String = "", val status: String = "", blockStatus: Boolean = false)
    ...
    override fun buildObservable(params: Params): Flowable<*> {
        return Flowable.just(params)
                ...
                .mapNotNull {
                    ...
                    var action : BlockingClient.Action? = null


                    val message = if (it.au){
                        action = BlockingClient.Action.Unblock
                        messageRepo.insertReceivedSms(it.subId, it.address, it.status, System.currentTimeMillis())
                    }else{
                        val messages = it.messages
                        val address = messages[0].displayOriginatingAddress
                        action = blockingClient.getAction(address).blockingGet()
                        ...
                        messageRepo.insertReceivedSms(it.subId, address, body, time)
                    }
                ...
                .doOnNext {
                    Log.v("SmsReceiver", "---------------Receiver-----Complete----Success--------")
                }
                .doOnError { Log.v("SmsReceiver", "---------------Receiver-----Error----${it.message}--------") }
    }
