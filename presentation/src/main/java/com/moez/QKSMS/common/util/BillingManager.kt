...
class BillingManager @Inject constructor(
    ...
    init {
        ...
        upgradeStatus = when (BuildConfig.FLAVOR) {
            "noAnalytics" -> BehaviorSubject.createDefault(true)
                    ...
                    .map { purchases -> purchases.any { it.skus[0] == SKU_PLUS } || purchases.any { it.skus[0] == SKU_PLUS_DONATE } }
    ...
    fun initiatePurchaseFlow(activity: Activity, sku: String) {
        executeServiceRequest {
            val params = BillingFlowParams.newBuilder().setSkuDetails(SkuDetails(sku))
    ...
    override fun onPurchasesUpdated(resultCode: BillingResult, purchases: MutableList<Purchase>?) {
